...
#define SLEEP_TIMEOUT 15000   // 15 seconds after OFF
bool sleeping=false;
...
void enterLightSleep(){
  ...
  esp_sleep_enable_timer_wakeup(500000);   // wake every 0.5s
...
void handleRF(){
  ...
  wakeUp(); // Wake from sleep mode when RF received. 
...
void loop(){
  ...
  updateLED();
   ...
   // if everything OFF → start sleep timer
  if(speedLevel==0 && !ledOn){
    if(!sleeping && millis()-lastActivity > SLEEP_TIMEOUT){
      enterLightSleep();
    }
  }else{
    wakeUp();  // if system running, stay awake
  }
